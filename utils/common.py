...
class NullableArgs:
    """Wraps an argparse.Namespace and returns None for missing attributes."""
...
def get_option_text(args: argparse.Namespace, parser: argparse.ArgumentParser) -> str:
    message = ''
    for k, v in sorted(vars(args).items()):
        ...
        message += f'{k!s:>30}: {v!s:<30}{comment}\n'
    return message
...
def get_pose_input(
    coef_dict: dict[str, torch.Tensor],
    rot_repr: str,
    with_global_pose: bool,
) -> torch.Tensor:
    if rot_repr == 'aa':
...
def get_motion_coef(
    ...
    if norm_stats is not None:
        if rot_repr == 'aa':
            ...
            raise ValueError(f'Unknown rotation representation {rot_repr}!')
        coef_dict = {
            k: (coef_dict[k] - norm_stats[f'{k}_mean']) / norm_stats[f'{k}_std'] for k in keys
        }
...
def get_coef_dict(
    ...
    shape_coef: torch.Tensor | None = None,
    denorm_stats: dict[str, torch.Tensor] | None = None,
    with_global_pose: bool = False,
    rot_repr: str = 'aa',
) -> dict[str, torch.Tensor]:
    coef_dict: dict[str, torch.Tensor] = {'exp': motion_coef[..., :50]}
    ...
    if rot_repr == 'aa':
        ...
        # Add back rotation around y, z axes
        coef_dict['pose'] = torch.cat(
            [coef_dict['pose'], torch.zeros_like(motion_coef[..., :2])], dim=-1
        )
    else:
        raise ValueError(f'Unknown rotation representation {rot_repr}!')
...
def coef_dict_to_vertices(
    coef_dict: dict[str, torch.Tensor],
    flame: nn.Module,
    rot_repr: str = 'aa',
    ignore_global_rot: bool = False,
    flame_batch_size: int = 512,
) -> torch.Tensor:
    ...
    for i in range(0, n_samples, flame_batch_size):
        ...
        if rot_repr == 'aa':
            vert, _, _ = flame(
                batch_coef_dict['shape'],
                batch_coef_dict['exp'],
                batch_coef_dict['pose'],
                pose2rot=True,
    ...
    vert_list_cat = torch.cat(vert_list, dim=0)  # (n_samples, 5023, 3)
    return vert_list_cat.view(*shape, -1, 3)  # (..., 5023, 3)


def compute_loss(
    args: Any,
    is_starting_sample: bool,
    shape_coef: torch.Tensor,
    motion_coef_gt: torch.Tensor,
    noise: torch.Tensor,
    target: torch.Tensor,
    prev_motion_coef: torch.Tensor,
    coef_stats: dict[str, torch.Tensor],
    flame: nn.Module,
    end_idx: torch.Tensor | None = None,
) -> tuple[
    torch.Tensor,
    torch.Tensor | None,
    torch.Tensor | None,
    ...
    if args.target == 'noise':
        ...
        if args.l_vert > 0 or args.l_vel > 0:
            coef_gt = get_coef_dict(
                motion_coef_gt, shape_coef, coef_stats, with_global_pose=False, rot_repr=args.rot_repr
            )
            coef_pred = get_coef_dict(
                target, shape_coef, coef_stats, with_global_pose=False, rot_repr=args.rot_repr
            )
            seq_len = target.shape[1]

            if args.rot_repr == 'aa':
                verts_gt, _, _ = flame(
                    coef_gt['shape'].view(-1, 100),
                    coef_gt['exp'].view(-1, 50),
                    coef_gt['pose'].view(-1, 6),
                    return_lm2d=False,
                    return_lm3d=False,
                )
                verts_pred, _, _ = flame(
                    coef_pred['shape'].view(-1, 100),
                    coef_pred['exp'].view(-1, 50),
                    coef_pred['pose'].view(-1, 6),
                    return_lm2d=False,
                    return_lm3d=False,
                )
            else:
        ...
        if not args.no_head_pose:
            ...
            if args.l_head_smooth > 0:
                head_vel_pred = head_pose_pred[:, 1:] - head_pose_pred[:, :-1]
                loss_head_smooth = criterion_func(
                    head_vel_pred[:, 1:], head_vel_pred[:, :-1], reduction='none'
                )
            ...
            if not is_starting_sample and args.l_head_trans > 0:
                ...
                loss_head_trans_vel = criterion_func(
                    head_vel_pred[:, 2:4], head_vel_pred[:, 1:3], reduction='none'
                )
                loss_head_trans_accel = criterion_func(
                    head_accel_pred[:, 1:], head_accel_pred[:, :-1], reduction='none'
                )
    else:
    ...
    if end_idx is None:
        ...
        mask = (
            torch.arange(args.n_motions, device=target.device).expand(target.shape[0], -1)
            < end_idx.unsqueeze(1)
        )
...
def truncate_coef_dict_and_audio(
    audio: torch.Tensor,
    coef_dict: dict[str, torch.Tensor],
    n_motions: int,
    audio_unit: int = 640,
...
def truncate_motion_coef_and_audio(
    audio: torch.Tensor,
    motion_coef: torch.Tensor,
    n_motions: int,
    audio_unit: int = 640,
...
def nt_xent_loss(
    ...
    Args:
        feature_a: (batch_size, feature_dim)
        feature_b: (batch_size, feature_dim)
    ...
    labels = (labels.unsqueeze(0) == labels.unsqueeze(1)).to(device)
