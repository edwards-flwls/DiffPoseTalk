...
class DiffusionSchedule(nn.Module):
    def __init__(
        ...
        betas = torch.cat([torch.zeros(1), betas], dim=0)  # Pad beta_0 = 0
    ...
    def get_sigmas(self, t: int, flexibility: float = 0) -> torch.Tensor:
...
class DiffTalkingHead(nn.Module):
    def __init__(self, args: Any, device: str | torch.device = 'cuda') -> None:
        ...
        self.use_style: bool = args.style_enc_ckpt is not None
        ...
        self.n_motions: int = args.n_motions
        self.n_prev_motions: int = args.n_prev_motions
        if self.use_style:
            self.style_feat_dim: int = args.d_style

        # Audio encoder — loaded lazily based on config to avoid importing both
        self.audio_model: str = args.audio_model
        if self.audio_model == 'wav2vec2':
            from .wav2vec2 import Wav2Vec2Model  # noqa: PLC0415

            self.audio_encoder = Wav2Vec2Model.from_pretrained('facebook/wav2vec2-base-960h')
            self.audio_encoder.feature_extractor._freeze_parameters()
        elif self.audio_model == 'hubert':
            from .hubert import HubertModel  # noqa: PLC0415

            self.audio_encoder = HubertModel.from_pretrained(
                'facebook/hubert-base-ls960', attn_implementation='eager'
            )
            self.audio_encoder.feature_extractor._freeze_parameters()
        ...
        if args.architecture == 'decoder':
            ...
            self.start_audio_feat = nn.Parameter(
                torch.randn(1, self.n_prev_motions, args.feature_dim)
            )
        else:
            raise ValueError(f'Unknown architecture {args.architecture}!')

        self.start_motion_feat = nn.Parameter(
            torch.randn(1, self.n_prev_motions, self.motion_feat_dim)
        )
        ...
        self.cfg_mode: str = args.cfg_mode
        guiding_conditions = args.guiding_conditions.split(',') if args.guiding_conditions else []
        self.guiding_conditions: list[str] = [
            cond for cond in guiding_conditions if cond in ['style', 'audio']
        ]
    ...
    def forward(
        ...
        time_step: list[int] | None = None,
        indicator: torch.Tensor | None = None,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Args:
            motion_feat: (N, L, d_coef)
            audio_or_feat: (N, L_audio) raw audio or (N, L, d_audio) audio feature
            shape_feat: (N, d_shape) or (N, 1, d_shape)
            style_feat: (N, d_style)
            prev_motion_feat: (N, n_prev_motions, d_motion)
            prev_audio_feat: (N, n_prev_motions, d_audio)
            time_step: (N,)
            indicator: (N, L) 0/1 indicator of real (unpadded) motion coefficients

        Returns:
            eps: (N, L, d_motion) sampled noise
            motion_feat_target: (N, L, d_motion) denoised target
            motion_feat: (N, L, d_motion) original motion (detached)
            audio_feat: (N, L, d_audio) audio features (detached)
        """
        if self.use_style:
        ...
        if len(self.guiding_conditions) > 0:
            assert len(self.guiding_conditions) <= 2, 'Only 1 or 2 CFG conditions supported!'
            if len(self.guiding_conditions) == 1 or self.cfg_mode == 'independent':
                ...
                if 'style' in self.guiding_conditions:
                    ...
                    style_feat = torch.where(
                        mask_style.view(-1, 1, 1),
                ...
                # incremental CFG: full (0.45), w/o style (0.45), w/o style or audio (0.1)
        ...
        motion_feat_target = self.denoising_net(
            motion_feat_noisy, audio_feat, person_feat, prev_motion_feat, prev_audio_feat, time_step, indicator
        )
    ...
    def extract_audio_feature(
        ...
        # Strategy: BackResample — resample after feature extraction
    ...
    def sample(
        ...
        prev_motion_feat: torch.Tensor | None = None,
        prev_audio_feat: torch.Tensor | None = None,
        motion_at_T: torch.Tensor | None = None,
        indicator: torch.Tensor | None = None,
        cfg_mode: str | None = None,
        cfg_cond: list[str] | None = None,
        cfg_scale: float | list[float] = 1.15,
        flexibility: float = 0,
        dynamic_threshold: tuple[float, float, float] | None = None,
        ret_traj: bool = False,
        ...
        if len(cfg_cond) > 0:
            cfg_cond_sorted, cfg_scale_sorted = zip(
                *sorted(zip(cfg_cond, cfg_scale), key=lambda x: ['audio', 'style'].index(x[0]))
            )
            cfg_cond = list(cfg_cond_sorted)
            cfg_scale = list(cfg_scale_sorted)
        else:
            cfg_cond, cfg_scale = [], []
        ...
        # Build batched inputs for CFG (null + conditional entries)
        ...
        if 'style' in cfg_cond:
            person_feat_null = torch.cat(
                [shape_feat, self.null_style_feat.expand(batch_size, -1, -1)], dim=-1
            )
        else:
            person_feat_null = (
                torch.cat([shape_feat, style_feat], dim=-1) if self.use_style else shape_feat
            )
        ...
        audio_feat_in_t = torch.cat(audio_feat_in, dim=0)
        person_feat_in_t = torch.cat(person_feat_in, dim=0)
        ...
        traj: dict[int, torch.Tensor] = {self.diffusion_sched.num_steps: motion_at_T}
        for t in range(self.diffusion_sched.num_steps, 0, -1):
            ...
            if dynamic_threshold:
                ...
                abs_results = (
                    results[:, -self.n_motions :].reshape(batch_size * n_entries, -1).abs()
                )
...
class DenoisingNetwork(nn.Module):
    def __init__(self, args: Any, device: str | torch.device = 'cuda') -> None:
        ...
        self.use_indicator: bool = args.use_indicator
        ...
        self.align_mask_width: int = args.align_mask_width
        self.use_learnable_pe: bool = not args.no_use_learnable_pe
        self.n_prev_motions: int = args.n_prev_motions
        self.n_motions: int = args.n_motions
        ...
        if self.use_learnable_pe:
            self.PE: nn.Parameter | PositionalEncoding = nn.Parameter(
        ...
        if self.architecture == 'decoder':
            self.feature_proj = nn.Linear(
                self.motion_feat_dim + (1 if self.use_indicator else 0), self.feature_dim
            )
    ...
    def forward(
        ...
        Args:
            ...
            prev_motion_feat: (N, L_p, d_motion)
            prev_audio_feat: (N, L_p, d_audio)
            step: (N,) diffusion time step indices
            indicator: (N, L) 0/1 indicator for real (unpadded) motion

        Returns:
            torch.Tensor: (N, L_p + L, d_motion) predicted motion target
        """
        diff_step_embedding = self.diff_step_map(
            self.TE.pe[0, step]
        ).unsqueeze(1)  # (N, 1, feature_dim)
        ...
        if indicator is not None:
            indicator = torch.cat(
                [
                    torch.zeros(
                        (indicator.shape[0], self.n_prev_motions), device=indicator.device
                    ),
                    indicator,
                ],
                dim=1,
            ).unsqueeze(-1)  # (N, L_p + L, 1)
